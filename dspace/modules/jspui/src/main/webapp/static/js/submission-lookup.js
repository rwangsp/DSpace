...
submissionLookupShowResult = function(info, suffixID){
	...
	{
		var bt = j('<button class="btn btn-success" type="button">').append(j('#jsseedetailsbuttonmessage').text());
		...
		for (var k=0;k<info.result[i].providers.length;k++)
		{
			...
			divImg.append(j('<img class="img-thumbnail" src="'+dspaceContextPath+'/image/submission-lookup-small-'+prov+'.jpg" alt="provider image thumbnail">'));
		}	
		par
