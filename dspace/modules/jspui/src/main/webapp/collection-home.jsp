...
<%-- <%
            ...
            {
%>
                : [<%= ic.getCount(collection) %>] //ITS customization removes this
<%
            }
%> --%>
...
<%  if (logo != null) { %>
        <div class="col-md-4">
        	<img id='collection-logo' class="img-responsive" alt="Logo" src="<%= request.getContextPath() %>/retrieve/<%= logo.getID() %>" />
        </div>
<% 	} %>
	</div> <%-- End Tag <div class="row"> --%>
<%
	if (StringUtils.isNotBlank(intro)) { %>
	   <%= intro %>
<% 	} %>
  </div> <%-- End Tag <div class="well"> --%>


  <p class="copyrightText"><%= copyright %></p>

  <%-- Browse --%>
  <div class="community-home-nav collection-home-nav">
    <%-- Insert the dynamic list of browse options --%>
    <h1>Browse items in this collection by the following</h1>
  ...
  <%
      ...
      %>
          ...
          <input type='hidden' name='starts_with' value='A' />
      		...
      		<input type="submit" class="btn btn-success" name="submit_browse" value="<fmt:message key="<%= key %>"/>"/>
      	</form>
      <%
  	}
  ...
  %>
<%-- </div> --%>
</div>  <%--End Tag class="community-home-nav collection-home-nav" --%>
...
<%  } %>
          <li>
              <a class="statisticsLink btn btn-success" href="<%= request.getContextPath() %>/handle/<%= collection.getHandle() %>/statistics">Statistics</a>
          </li>
        ...
        <%  if (loggedIn && subscribed)
            { %>
                <input class="btn btn-success" type="submit" name="submit_unsubscribe" value="<fmt:message key="jsp.collection-home.unsub"/>" />
        <%  } else { %>
    ...
    <li>
        <a class='btn btn-success' href="<%= request.getContextPath() %>/feed/<%= fmts[j] %>/<%= collection.getHandle() %>"><%= icon %></a>
    </li>
    <%
        } // End of for loop
          %>
...
%>
    ...
    <div align="center" style="overflow-x:auto;background:whitesmoke; border:1px solid #efefef; border-radius:4px; text-align:left; padding:1%;">
...
%>
    ...
    <div align="center" style="background:whitesmoke; border:1px solid #efefef; border-radius:4px; text-align:left; padding:1%;">
...
<% if(admin_button || editor_button ) { %>
                 <div class="panel panel-warning admin-tools">
                 <div class="panel-heading">
